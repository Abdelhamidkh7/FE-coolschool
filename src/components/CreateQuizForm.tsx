...
const CreateQuizForm: React.FC<CreateQuizFormProps> = ({ classId, onSuccess }) => {
  ...
  const updateChoice = (
    ...
    choices[cIndex] = { ...choices[cIndex], [key]: value };
  ...
  const handleSubmit = async () => {
    try {
      const payload = {
        ...
        startDate,
        endDate,
        questions: quizType === "QUIZ" ? questions : [], // if "ASSIGNMENT", no questions
      };

      await axios.post(`http://localhost:8080/api/quiz`, payload, {
        headers: {
  ...
  return (
    <div className="p-6 bg-white shadow-xl rounded-lg max-w-3xl mx-auto">
      ...
      <div className="mb-4">
        ...
        <select
          value={quizType}
          onChange={(e) => setQuizType(e.target.value as "QUIZ" | "ASSIGNMENT")}
          className="w-full border rounded p-2 mt-1"
        >
      ...
      {/* Only show question builder if it's a "QUIZ" */}
      {quizType === "QUIZ" && (
        <>
                    ...
                    <div key={cIndex} className="flex items-center space-x-2 mb-2">
                      ...
                      <label className="flex items-center space-x-1">
                        <input
                          ...
                          checked={c.isCorrect}
                          onChange={(e) =>
                            updateChoice(index, cIndex, "isCorrect", e.target.checked)
                          }
                        />
